...
export const createPreparedStatements = (db) => {
  return {
    ...
    getMetadata: db.prepare(
      'SELECT id, title, language, timestamp, type, tags, is_draft, sort_index, folder_id, is_pinned FROM snippets WHERE is_deleted = 0 ORDER BY is_pinned DESC, COALESCE(sort_index, 0) ASC, title COLLATE NOCASE ASC'
    ),
    getMetadataPaginated: db.prepare(
      'SELECT id, title, language, timestamp, type, tags, is_draft, sort_index, folder_id, is_pinned FROM snippets WHERE is_deleted = 0 ORDER BY is_pinned DESC, COALESCE(sort_index, 0) ASC, title COLLATE NOCASE ASC LIMIT ? OFFSET ?'
    ),
    ...
    getRecent: db.prepare(
      'SELECT id, title, language, timestamp, type, tags, is_draft, sort_index, folder_id, is_pinned FROM snippets WHERE is_draft = 0 AND is_deleted = 0 ORDER BY timestamp DESC LIMIT ?'
    ),
    getTrash: db.prepare(
    ...
    save: db.prepare(`
      ...
      (id, title, code, language, timestamp, type, tags, is_draft, sort_index, folder_id, is_deleted, is_pinned)
      VALUES (@id, @title, @code, @language, @timestamp, @type, @tags, @is_draft, @sort_index, @folder_id, 0, @is_pinned)
    `),
