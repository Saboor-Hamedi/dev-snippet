...
In large snippet libraries (10,000+ entries), the standard Full-Text Search (FTS5) would begin to exhibit "UI micro-stutters." This was because the `snippet()` function—which generates the highlighted context—was being executed for *every match* found by SQLite before sorting and limiting, leading to unnecessary CPU spikes in the Main Process.
...
2. **Deferred Content Fetching**: The expensive `snippet()` generation and table joins now only occur *after* the `LIMIT 10` has been applied to the ranked results.
...
1. **RAF Throttling**: The `onScroll` handler no longer triggers a React state update immediately. Instead, it uses `requestAnimationFrame` (RAF) to schedule the update for the *next paint cycle*.
